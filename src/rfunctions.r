...
##
TEA.hpc <- function (sorted.genes, target.sets, nperm = 1000, all = F,
                      weighted.score.type=0, ## 0:NULL or 1:actvity.scores to use as correl.vector
                      ## parameters related to TEA up to here
                      n.split=256, ## n.split is explicitly specified in TEA
    ...
    n.gs <- length(target.sets)
    inds <- sort(rep(seq(n.split),length.out=n.gs))
    x <- lapply(seq(n.split),function(i){
        tgt.sets <- target.sets[inds==i]
        save(tgt.sets,file=paste("tgts",sprintf("%03d",i),".rda",sep=""))
    })
